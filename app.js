...
const SNAP_CFG=[
  {label:'Wake Up',sub:'Log sleep and recovery from this morning.',fields:[
    {id:'sleep',label:'Sleep (hrs)',ph:'e.g. 7.5',step:'0.5'},
...
function renderWhoopSnap(){
  ...
  const items=cfg.fields.map(f=>{
    ...
    const d=f.id==='steps'?Number(v).toLocaleString():f.id==='recovery'?v+'%':f.id==='sleep'?v+'h':v;
...
function saveWhoop(){
  const cfg=SNAP_CFG[activeTab];
  const snap={};
  cfg.fields.forEach(f=>{const el=gv(`wf-${f.id}`);if(el&&el.value!=='')snap[f.id]=f.id==='steps'?parseInt(el.value):parseFloat(el.value);});
...
function renderQuickAdd(){
  const el=gv('qa-scroll');let html='';
  for(const item of quickItems){
    html+=`<div class="qa-item" onclick="quickLog('${item.id}')"><div class="qa-icon">${item.emoji}</div><div class="qa-name">${item.name}</div><div class="qa-kcal">${item.calories} kcal</div></div>`;
  }
  html+=`<div class="qa-add" onclick="openQAModal()"><div style="font-size:20px">＋</div><div style="font-size:9px">Add</div></div>`;
  el.innerHTML=html;
}
function quickLog(id){
  ...
  save(`${KEY}_meals_${todayKey()}`,meals);renderAll();
}
...
function confirmMeal(){
  ...
  const t=sel.reduce((a,i)=>({cal:a.cal+i.calories*i.portion_multiplier,p:a.p+i.protein*i.portion_multiplier,c:a.c+i.carbs*i.portion_multiplier,f:a.f+i.fat*i.portion_multiplier}),{cal:0,p:0,c:0,f:0});
  meals.push({name:sel.length===1?sel[0].name:sel.map(i=>i.name).join(' + '),
    emoji:sel[0].emoji,calories:Math.round(t.cal),protein:Math.round(t.p*10)/10,
  ...
  setTimeout(()=>{resetLogModal();showPage('today',document.querySelectorAll('.nb')[0]);renderAll();},100);
}
