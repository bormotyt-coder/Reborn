...
function renderNutrients(){
  ...
  const fibreCol=fibre>=25?'var(--lime)':fibre>=15?'var(--amber)':'var(--muted)';
...
// ══════════════════════════════════════════════════════════════════════════
// FEATURE: BARCODE SCANNER — uses Open Food Facts API (free, no key)
// ══════════════════════════════════════════════════════════════════════════
...
function openBarcodeModal(){
  ...
  gv('barcode-status').textContent='Point camera at barcode, or type it below';
  startBarcodeCamera();
}
...
async function startBarcodeCamera(){
  ...
  if(!video)return;
  try{
    barcodeStream=await navigator.mediaDevices.getUserMedia({video:{facingMode:'environment',width:{ideal:1280}}});
    video.srcObject=barcodeStream;
    video.play();
    // Use BarcodeDetector if supported (Chrome on Android, Safari 17.4+)
    if('BarcodeDetector' in window){
      const detector=new BarcodeDetector({formats:['ean_13','ean_8','upc_a','upc_e','code_128','code_39','qr_code']});
      const scan=setInterval(async()=>{
        if(!barcodeStream){clearInterval(scan);return;}
        try{
          const barcodes=await detector.detect(video);
          if(barcodes.length){
            clearInterval(scan);
            const code=barcodes[0].rawValue;
            gv('barcode-manual').value=code;
            lookupBarcode(code);
          }
        }catch(e){}
      },400);
    } else {
      gv('barcode-status').textContent='Auto-scan not supported on this device. Type barcode number below.';
    }
  }catch(e){
    gv('barcode-status').textContent='Camera unavailable. Type barcode number below.';
  }
}
...
async function lookupBarcode(code){
  ...
  try{
    ...
    if(data.status!==1||!data.product){
      gv('barcode-status').textContent='Product not found. Try the AI lookup instead.';
      return;
    }
    ...
    gv('barcode-status').textContent='✓ Product found';
    stopBarcodeCamera();
  }catch(e){
    gv('barcode-status').textContent='Lookup failed. Check connection.';
    console.error(e);
  }
}
...
function renderWeekly(){
  ...
  <div class="week-stats-row">
    <div class="wk-s"><div class="wk-v" style="color:var(--lime)">${avgCal||'—'}</div><div class="wk-l">avg kcal/day</div></div>
  ...
  days.forEach(d=>{
    ...
    const col=d.t.cal===0?'var(--glass2)':d.t.cal>tgt*1.1?'var(--red)':d.t.cal>=tgt*0.85?'var(--lime)':'var(--amber)';
    const borderStyle=d.isToday?'border:2px solid var(--lime);border-bottom:none;':'';
    html+=`<div class="wk-bar-wrap${d.isToday?' today':''}">
      ...
      <div class="wk-bar-dot" style="background:${d.hasData?'var(--lime)':'var(--glass2)'}"></div>
    </div>`;
  });
  html+=`</div>`;
  ...
  [...days].reverse().forEach(d=>{
    ...
    html+=`<div class="week-day-row">
      ...
      <div class="wkd-stats">
        <span style="color:var(--lime)">${Math.round(d.t.cal)||'—'} kcal</span>
    ...
    for(const s of parsed.suggestions){
      ...
      div.innerHTML=`
        ...
        <div class="sugg-macros">
          <span style="color:var(--lime)">${s.calories} kcal</span>
