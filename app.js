...
// BOOT
const _n=new Date();
if(gv('hdr-date'))gv('hdr-date').innerHTML=_n.toLocaleDateString('en-US',{weekday:'short',month:'short',day:'numeric'})+'<br>'+_n.toLocaleTimeString('en-US',{hour:'2-digit',minute:'2-digit'});
if(gv('coach-date'))gv('coach-date').textContent=_n.toLocaleDateString('en-US',{weekday:'long',month:'long',day:'numeric'});
renderAll();
initWelcomeCard();
...
function showPage(id,btn){
  ...
  gv('pg-'+id).classList.add('active');
  if(btn)btn.classList.add('active');
  gv('pg-'+id).scrollTop=0;
  if(id==='calendar')buildCalendar();
  if(id==='coach')updateCoachStats();
  // FAB: hide on coach page (already there), show elsewhere
  const fab=gv('coach-fab');
  if(fab)fab.classList.toggle('hidden',id==='coach');
}
function openCoach(){
  document.querySelectorAll('.page').forEach(p=>p.classList.remove('active'));
  document.querySelectorAll('.nb').forEach(b=>b.classList.remove('active'));
  gv('pg-coach').classList.add('active');
  gv('pg-coach').scrollTop=0;
  updateCoachStats();
  const fab=gv('coach-fab');if(fab)fab.classList.add('hidden');
}
...
function renderWhoopStrip(){
  // Compact strip on Today tab — shows most useful current stats inline
  const el=gv('ws-stats');if(!el)return;
  // Gather all available stats across snaps for strip display
  const stats=[];
  const s0=whoopSnaps[0];
  if(s0){
    if(s0.sleep!=null){const hh=Math.floor(s0.sleep),mm=Math.round((s0.sleep-hh)*60);stats.push({v:hh+'h'+(mm>0?' '+mm+'m':''),l:'Sleep'});}
    if(s0.recovery!=null)stats.push({v:s0.recovery+'%',l:'Recovery'});
  }
  const sLatest=whoopSnaps[2]||whoopSnaps[1];
  if(sLatest){
    if(sLatest.strain!=null)stats.push({v:sLatest.strain,l:'Strain'});
    if(sLatest.burned!=null)stats.push({v:sLatest.burned,l:'Burned'});
    if(sLatest.steps!=null)stats.push({v:Number(sLatest.steps).toLocaleString(),l:'Steps'});
  }
  if(!stats.length){el.innerHTML='<div class="ws-empty">Tap + to log</div>';return;}
  el.innerHTML=stats.map(s=>`<div class="ws-stat"><div class="ws-stat-v">${s.v}</div><div class="ws-stat-l">${s.l}</div></div>`).join('');
}
...
// ── WEEK STRIP (Today page) ──────────────────────
function renderWeekStrip(){
  const el=gv('week-strip');if(!el)return;
  const now=new Date();
  const today=now.toISOString().slice(0,10);
  let html='';
  // Show Mon–Sun of current week based on today
  for(let i=-3;i<=3;i++){
    const d=new Date(now);d.setDate(now.getDate()+i);
    const ds=d.toISOString().slice(0,10);
    const lbl=d.toLocaleDateString('en-US',{weekday:'short'}).slice(0,2).toUpperCase();
    const num=d.getDate();
    const hm=load(`${KEY}_meals_${ds}`,[]).length>0;
    const hw=load(`${KEY}_whoopsnaps_${ds}`,[null,null,null]).some(s=>s!==null);
    const isToday=ds===today;
    const isSel=ds===_stripSel;
    const cls=['ws-day',isToday?'today':'',isSel?'selected':'',hm||hw?'has-data':''].filter(Boolean).join(' ');
    html+=`<div class="${cls}" onclick="selectStripDay('${ds}')"><div class="ws-day-lbl">${lbl}</div><div class="ws-day-num">${num}</div><div class="ws-day-dot"></div></div>`;
  }
  el.innerHTML=html;
  renderStripDetail();
}
let _stripSel=null;
function selectStripDay(ds){
  const today=new Date().toISOString().slice(0,10);
  if(ds===today){_stripSel=null;}else{_stripSel=_stripSel===ds?null:ds;}
  renderWeekStrip();
}
function renderStripDetail(){
  const el=gv('strip-detail');if(!el)return;
  if(!_stripSel){el.style.display='none';return;}
  const dm=load(`${KEY}_meals_${_stripSel}`,[]);
  const dc=parseInt(localStorage.getItem(`${KEY}_cups_${_stripSel}`)||'0');
  const dw=load(`${KEY}_whoopsnaps_${_stripSel}`,[null,null,null]);
  const t=getTotals(dm);
  const d=new Date(_stripSel+'T12:00:00');
  const lbl=d.toLocaleDateString('en-US',{weekday:'long',month:'short',day:'numeric'});
  let html=`<div style="font-size:12px;font-weight:600;color:var(--text2);margin-bottom:10px;letter-spacing:-.1px">${lbl}</div>`;
  html+=`<div style="display:grid;grid-template-columns:repeat(5,1fr);gap:6px;margin-bottom:${dm.length?'10px':'0'}">`;
  html+=`<div class="ddc-s"><div class="ddc-v" style="color:var(--blue2)">${t.cal>0?Math.round(t.cal):'—'}</div><div class="ddc-l">kcal</div></div>`;
  html+=`<div class="ddc-s"><div class="ddc-v" style="color:var(--pc)">${t.p>0?Math.round(t.p)+'g':'—'}</div><div class="ddc-l">Protein</div></div>`;
  html+=`<div class="ddc-s"><div class="ddc-v" style="color:var(--cc)">${t.c>0?Math.round(t.c)+'g':'—'}</div><div class="ddc-l">Carbs</div></div>`;
  html+=`<div class="ddc-s"><div class="ddc-v" style="color:var(--fc)">${t.f>0?Math.round(t.f)+'g':'—'}</div><div class="ddc-l">Fat</div></div>`;
  html+=`<div class="ddc-s"><div class="ddc-v" style="color:var(--wc)">${dc||'—'}</div><div class="ddc-l">💧</div></div>`;
  html+=`</div>`;
  if(dm.length){
    ...
    html+=`<div style="font-size:11px;color:var(--muted);padding-top:2px">No meals logged.</div>`;
  }
  el.innerHTML=html;
...
Time of day: ${new Date().getHours()}:00. Goal: fat loss cut phase.`;
  ...
  try{
    ...
    el.innerHTML='<div style="color:var(--muted);font-size:12px;padding:8px 0">Suggestions failed. Try again.</div>';el.style.display='block';
  }finally{
    // btn removed — no-op
  }
}
