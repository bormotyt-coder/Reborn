...
const SNAP_CFG=[
  {label:'Wake Up',sub:'Log sleep and recovery from this morning.',fields:[
    {id:'sleep',label:'Sleep',ph:'e.g. 7:42',step:'',type:'sleep'},
...
function renderWhoopSnap(){
  ...
  const items=cfg.fields.map(f=>{
    ...
    const d=f.id==='steps'?Number(v).toLocaleString():f.id==='recovery'?v+'%':f.id==='sleep'?sleepDecimalToHHMM(v)+'h':v;
...
// Convert decimal sleep hours to HH:MM string
function sleepDecimalToHHMM(dec){
...
function sleepHHMMToDecimal(str){
  if(!str||!str.includes(':'))return parseFloat(str)||null;
...
function openWhoopModal(){
  ...
  for(let i=0;i<cfg.fields.length;i+=2){
    ...
    for(let j=i;j<Math.min(i+2,cfg.fields.length);j++){
      ...
      if(f.type==='sleep'){
        const val=sleepDecimalToHHMM(snap[f.id]);
        html+=`<div><label class="flbl">${f.label} <span style="color:var(--muted);font-size:10px;font-weight:400">h:mm</span></label><input type="text" id="wf-${f.id}" placeholder="${f.ph}" value="${val}" inputmode="numeric" style="letter-spacing:.05em"/></div>`;
      } else {
...
function saveWhoop(){
  ...
  cfg.fields.forEach(f=>{
    ...
    if(f.type==='sleep'){
      snap[f.id]=sleepHHMMToDecimal(el.value);
    } else {
...
function renderQuickAdd(){
  ...
  for(const item of quickItems){
    ...
    div.innerHTML=`<div class="qa-icon">${item.emoji}</div><div class="qa-name">${item.name}</div><div class="qa-kcal">${item.calories} kcal</div><button class="qa-del" onclick="event.stopPropagation();deleteQAItem('${item.id}')" title="Remove">✕</button>`;
...
function quickLog(id){
  ...
  renderAll();
  setTimeout(()=>{_quickLogCooldown=false;},600);
}
...
let _confirmingMeal=false;
function confirmMeal(){
  if(_confirmingMeal)return;
  const sel=ingredients.filter(i=>i.selected);
  if(!sel.length){alert('Select at least one ingredient.');return;}
  _confirmingMeal=true;
  const t=sel.reduce((a,i)=>({cal:a.cal+i.calories*i.portion_multiplier,p:a.p+i.protein*i.portion_multiplier,c:a.c+i.carbs*i.portion_multiplier,f:a.f+i.fat*i.portion_multiplier}),{cal:0,p:0,c:0,f:0});
  const entry={
    ...
    thumb:mealB64?`data:image/jpeg;base64,${mealB64}`:null
  };
  meals.push(entry);
  save(`${KEY}_meals_${todayKey()}`,meals);
